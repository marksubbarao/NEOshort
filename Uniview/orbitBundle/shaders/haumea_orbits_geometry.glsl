...
uniform float wideAlpha;
uniform float narrowAlpha;
...
void main()
{
	...
	float i = gl_in[0].gl_Position.z*TWOPI/360.;
 	float OMEGA = gl_in[1].gl_Position.y;
	float omega = gl_in[1].gl_Position.x;
	float M = gl_in[1].gl_Position.z;
	float Hmag = gl_in[2].gl_Position.x;
	int type = int(gl_in[2].gl_Position.y);
	float discovery = gl_in[2].gl_Position.z;

	
	if (type==1) {
	  color=vec4(1.0,0.0,0.0,wideAlpha*uv_fade);
	} else {
	  color=vec4(0.0,1.0,1.0,narrowAlpha*uv_fade);
	}
